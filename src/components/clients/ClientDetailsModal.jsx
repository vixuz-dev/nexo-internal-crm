...
import { FiMapPin, FiPhone, FiMail, FiCalendar, FiTag, FiEdit2, FiSave, FiX } from 'react-icons/fi';
...
const ClientDetailsModal = ({ isOpen, onClose, client }) => {
  ...
  const { addClient, updateClientInList } = useClientsList();
  ...
  useEffect(() => {
    if (client) {
      setFormData({
        name: client.name || '',
        personal_phonenumber: client.personal_phonenumber || '',
        email: client.email || '',
      ...
      // Modo create: inicializar con valores vacíos
      setFormData({
        name: '',
  ...
  const validateForm = () => {
    ...
    // Validar nombre (requerido en modo create)
    if (isCreateMode && (!formData.name || formData.name.trim() === '')) {
      newErrors.name = 'El nombre es requerido';
    }

    // Validar teléfono (requerido y debe tener 10 dígitos)
    if (!formData.personal_phonenumber || formData.personal_phonenumber.trim() === '') {
      newErrors.personal_phonenumber = 'El teléfono es requerido';
    } else {
      const phoneError = validatePhone(formData.personal_phonenumber);
  ...
  const handleCancel = () => {
    if (client) {
      ...
      // Modo create: resetear formulario y cerrar modal
      setFormData({
        name: '',
        personal_phonenumber: '',
        email: '',
      ...
      // Cerrar el modal en modo create
      onClose();
    }
    setErrors({});
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (isCreateMode) {
        ...
        const newClient = response?.body || response?.data;
        if (newClient) {
          addClient(newClient);
          onClose();
        }
  ...
  return (
    <Modal
      ...
      title={isCreateMode ? "Nuevo Cliente" : (isEditing ? "Editar Cliente" : "Información del Cliente")}
      size="lg"
    >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className={isCreateMode ? "md:col-span-3" : "md:col-span-2"}>
                  <label className="block text-sm font-poppinsMedium text-neutral-700 mb-2">
                    Nombre {isCreateMode && <span className="text-red-500">*</span>}
                  </label>
                  <div className="relative">
                    <input
                      ...
                      onBlur={(e) => {
                        if (isCreateMode && (!e.target.value || e.target.value.trim() === '')) {
            ...
            <div className="md:col-span-1">
              ...
              <div className="relative">
                <input
                  ...
                  readOnly={!isEditing}
                  onChange={(e) => handleInputChange('personal_phonenumber', e.target.value)}
                  onBlur={(e) => handleBlur('personal_phonenumber', e.target.value)}
                  className={`w-full px-4 py-2 border-2 rounded-lg font-poppinsRegular ${
              ...
              {errors.personal_phonenumber && (
                <p className="mt-1 text-sm text-red-600">{errors.personal_phonenumber}</p>
              )}
            </div>
            ...
            <div className="md:col-span-1">
              <Select
                ...
                readOnly={!isEditing}
                readOnlyClassName={!isCreateMode && client?.status === 1 ? 'text-emerald-700' : (!isCreateMode && client?.status === 0 ? 'text-rose-700' : '')}
                optionValue="value"
                optionLabel="label"
              />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="md:col-span-1">
              ...
              <input
                ...
                readOnly={!isEditing}
                onChange={(e) => handleInputChange('external_number', e.target.value)}
                className={`w-full px-4 py-2 border-2 border-neutral-500 rounded-lg font-poppinsRegular ${
                  isEditing 
                    ? 'bg-white text-neutral-900 focus:ring-2 focus:ring-primary-500 focus:border-primary-500' 
                    : 'bg-neutral-50 text-neutral-900'
              ...
              <Select
                ...
                onChange={(value) => {
                  ...
                  if (value && value.trim() !== '') {
                    ...
                    setErrors(prev => ({ ...prev, state: 'El estado es requerido' }));
                  }
                }}
                onBlur={(e) => {
                ...
                options={estadosMexico.map(estado => ({ value: estado, label: estado }))}
              ...
              {isEditing ? (
                <>
                  ...
                  >
                    ...
                    {isSaving ? 'Guardando...' : (isCreateMode ? 'Crear Cliente' : 'Guardar Cambios')}
                  </button>
                </>
              ) : (
                <>
